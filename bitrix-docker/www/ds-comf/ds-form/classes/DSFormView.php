...
class DSFormView extends DSMain {
	...
	public function index() {
		...
		foreach ($this->formConfig->formFields as $index => $field) {
			...
			if (isset($field['container']) && empty($field['container'])) {
				$strForm .= $this->getTag($field);
			} else {
	...
	private function getTag($field) {
		switch ($field['type']) {
			case 'input':
				return $this->input($field['label'], $field['attributs']);
			break;
			case 'textarea':
				return $this->textarea($field['label'], $field['attributs']);
			break;
			case 'select':
				return $this->select($field['label'], $field['attributs'], $field['options']);
			break;
			case 'freearea':
				return "\n" . $field['value'];
			break;
    ...
    private function changeValue($attributs, $fieldType, $options = array()) {
    	$changeResult = array();
 	...
 	private function input($label, $attributs) {
		$input = '';
    ...
    private function textarea($label, $attributs) {
    	$textarea ="";
    ...
    private function select($label, $attributs, $options) {
		$select = '';
