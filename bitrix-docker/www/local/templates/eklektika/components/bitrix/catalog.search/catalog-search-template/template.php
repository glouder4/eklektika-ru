...
if (!empty($searchFilter) && is_array($searchFilter))
{
    ...
    // Обработка параметров фильтра из URL (аналогично section_main_catalog_template-view.php)
    ...
    if (isset($_GET) && is_array($_GET))
    {
        ...
        if (isset($_GET['f8']) && $_GET['f8'] !== '')
        {
            ...
            if (preg_match('/^minmax~(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)$/', $priceFilter, $matches))
            {
                ...
                // Получаем ID типа цены из параметров
...
// Показываем смартфильтр для поиска
    if (CModule::IncludeModule("iblock") && CModule::IncludeModule("catalog") && (isset($_GET['q']) || !empty($_GET['set_filter']))):
        ?>
